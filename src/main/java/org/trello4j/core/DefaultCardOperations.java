...
public class DefaultCardOperations extends AbstractOperations implements CardOperations {
    ...
    @Override
    public Label createLabel(String color, String name, String... filters) {
        Map<String, Object> keyValueMap = new HashMap<String, Object>();
        keyValueMap.put("color", color);
        keyValueMap.put("name", name);

        TrelloURI uri = getTrelloAccessor().createTrelloUri(TrelloURI.CARD_POST_CREATE_LABELS, cardId).filter(filters);

        return getTrelloAccessor().doPost(uri.build(), keyValueMap, Label.class);
    }
    ...
    public boolean deleteChecklist(String listId, String... filters) {
        TrelloURI uri = getTrelloAccessor().createTrelloUri(TrelloURI.CARD_DELETE_CHECKLIST, cardId, listId)
                .filter(filters);
        return getTrelloAccessor().doDelete(uri.build());
    }
    ...
    public boolean deleteMember(String memberId, String... filters) {
        TrelloURI uri = getTrelloAccessor().createTrelloUri(TrelloURI.CARD_DELETE_MEMBER, cardId, memberId)
                .filter(filters);
        return getTrelloAccessor().doDelete(uri.build());
    }
    ...
    public boolean deleteVote(String memberId, String... filters) {
        TrelloURI uri = getTrelloAccessor().createTrelloUri(TrelloURI.CARD_DELETE_VOTE_MEMBER, cardId, memberId)
                .filter(filters);
        return getTrelloAccessor().doDelete(uri.build());
    }
